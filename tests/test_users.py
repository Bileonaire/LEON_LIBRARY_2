...
class UsersTests(BaseTests):
    ...
    def test_user_get_all_no_token(self):
        """Tests normal user unauthorized to get get all users"""
        response = self.app.get('/api/v2/users')
        self.assertEqual(response.status_code, 401)
    
    def test_user_get_all_fake_token(self):
        """Tests normal user unauthorized to get get all users"""
        response = self.app.get('/api/v2/users', headers={"Content-Type" : "application/json", "x-access-token" : "jjjjjj"})
        self.assertEqual(response.status_code, 401)
    ...
    def test_invalid_token_admin(self):
        ...
        invalid_token = {
            ...
            "x-access-token" : "eyJ0eXAiOiJKv2QiLCJhbGciOiJIUzI1NiJ9.eyJpZCI6NCwiYWRtaW4iOnRydWUsImV4cCI6MTUyNjczNzQ5Nvm2laNiJek7X266RLLk-bWL-ZF2RuD32FBvg_G8KyM"}
