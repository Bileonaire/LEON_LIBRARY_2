...
class BaseTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with self.application.app_context():
            ...
            book = json.dumps({"book" : "book1", "author" : "messi", "copies" : 4 })
            borrowed = json.dumps({"borrowed_book" : "book1"})
            create_book = self.app.post(
                '/api/v2/books', data=book, content_type='application/json',
                headers=self.admin_header)
            create_borrowed = self.app.post(
                '/api/v2/borrowed', data=borrowed, content_type='application/json',
